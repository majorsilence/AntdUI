        ...
        void ChangeBeforeFormBorderColor(bool dark)
        {
            ...
            if (form is Window window)
            {
                ...
                else if (formBorderColor is string code)
                {
                    ...
                    {
                        case "border":
                            window.BorderColor = AntdUI.Style.Get(Colour.BorderColor, dark ? TAMode.Dark : TAMode.Light, nameof(Window), window.Name);
                            break;
                        case "primary":
                            window.BorderColor = AntdUI.Style.Get(Colour.Primary, dark ? TAMode.Dark : TAMode.Light, nameof(Window), window.Name);
                            break;
                    }
                }
            }
            else if (form is BorderlessForm borderlessForm)
            {
                if (borderlessForm.UseDwm || borderlessForm.BorderWidth > 0)
                {
                    ...
                    else if (formBorderColor is string code)
                    {
                        ...
                        {
                            case "border":
                                borderlessForm.BorderColor = AntdUI.Style.Get(Colour.BorderColor, dark ? TAMode.Dark : TAMode.Light, nameof(BorderlessForm), borderlessForm.Name);
                                break;
                            case "primary":
                                borderlessForm.BorderColor = AntdUI.Style.Get(Colour.Primary, dark ? TAMode.Dark : TAMode.Light, nameof(BorderlessForm), borderlessForm.Name);
                                break;
                        }
                    }
                }
