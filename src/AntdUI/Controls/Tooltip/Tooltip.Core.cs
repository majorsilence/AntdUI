...
namespace AntdUI
{
    internal static class ITooltipLib
    {
        ...
        public static void Render(this ITooltip core, Canvas g, Rectangle rect, bool multiline, int arrowSize, int arrowX, FormatFlags s_c, FormatFlags s_l)
        {
            ...
            using (var brush = new SolidBrush(core.Back ?? AntdUI.Style.Get(Colour.BgSpotlight, nameof(Tooltip))))
            {
                if (core.ArrowAlign == TAlign.None)
                {
                    ...
                    g.DrawText(core.Text, core.Font, core.Fore ?? AntdUI.Style.Get(Colour.TextSpotlight, nameof(Tooltip)), rect_text, multiline ? s_l : s_c);
                }
                else
                {
